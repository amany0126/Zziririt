...
<div class="container" id="contentDiv" align="center">
	<div class="outer">
		...
		<br>
		<h2 align="center">보낸 쪽지함 (<%=listCountSe %>)</h2>
		<br>
		
		<form action="letterSearch.lo" method="GET">
		<input type="hidden" name="letterStatus" value="2">
		<input type="hidden" name="currentPage" value="1">
        <input type="text" id="keyword" name="keyword" placeholder="제목 / 수신자 검색" style="width:280px">
		...
		<div align="center">
			...
			<form id="deleteForm" action="lettersDelete.lo?letterStatus=2" method="post">
			...
			<button id="receiveLetter" onclick="location.href = '<%= contextPath %>/letterList.lo?currentPage=1&letterStatus=2'">발신함(<%=listCountSe %>)</button>
		...
		<div align="center" class="paging-area">
			...
			<% if(currentPage != 1) { %>
				...
				<!-- 이전페이지로 이동 버튼이 보여져야함 -->
				<button 
					onclick="location.href = '<%= contextPath %>/letterList.lo?currentPage=<%= currentPage - 1 %>&letterStatus=2';">
					&lt;
				</button>
			...
			<% for(int p = startPage; p <= endPage ; p++) { %>
				...
				<% if(p != currentPage) { %>
					...
					<button 
						onclick="location.href = '<%= contextPath %>/letterList.lo?currentPage=<%= p %>&letterStatus=2';">
						<%= p %>
					</button>
