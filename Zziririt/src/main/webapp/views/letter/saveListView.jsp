...
<div class="container" id="contentDiv" align="center">
	<div class="outer">
		...
		<br>
		<h2 align="center">임시 보관함 (<%=listCountSa%>)</h2>
		<br>
				...
				<% } else { %>
					...
					<% for(Letter l : list) { %>
						...
						<tr>
							...
							<td><%= l.getLetterTitle()==null?"제목없음":l.getLetterTitle() %></td>
							<td><%= l.getSaveReceiver()==null?"대상없음":l.getSaveReceiver() %></td>
							<td><%= l.getSendTime() %></td>
						</tr>
		...
		<div align="center" class="paging-area">
			...
			<% if(currentPage != 1) { %>
				...
				<!-- 이전페이지로 이동 버튼이 보여져야함 -->
				<button 
					onclick="location.href = '<%= contextPath %>/letterList.lo?currentPage=<%= currentPage - 1 %>&letterStatus=4';">
					&lt;
				</button>
			...
			<% for(int p = startPage; p <= endPage ; p++) { %>
				...
				<% if(p != currentPage) { %>
					...
					<button 
						onclick="location.href = '<%= contextPath %>/letterList.lo?currentPage=<%= p %>&letterStatus=4';">
						<%= p %>
					</button>
