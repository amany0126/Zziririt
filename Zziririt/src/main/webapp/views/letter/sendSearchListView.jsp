...
<div class="container" id="contentDiv" align="center">
	<div class="outer">
		...
		<br>
		<h2 align="center">보낸 쪽지함 (검색결과: <%=listCount%>)</h2>
		<br>
		...
		<div align="center" class="paging-area">
			...
			<% if(currentPage != 1) { %>
				...
				<!-- 이전페이지로 이동 버튼이 보여져야함 -->
				<button 
					onclick="location.href = '<%= contextPath %>/letterSearch.lo?currentPage=<%= currentPage - 1 %>&letterStatus=2&keyword=<%=keyword%>';">
					&lt;
				</button>
			...
			<% for(int p = startPage; p <= endPage ; p++) { %>
				...
				<% if(p != currentPage) { %>
					...
					<button 
						onclick="location.href = '<%= contextPath %>/letterSearch.lo?currentPage=<%= p %>&letterStatus=2&keyword=<%=keyword%>';">
						<%= p %>
					</button>
