<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@page
	import="com.zziririt.common.model.vo.PageInfo ,java.util.ArrayList, com.zziririt.admin.model.vo.GeneralBulletinComment"%>
<%
// 응답데이터 뽑기
ArrayList<String> categoryName = (ArrayList<String>) request.getAttribute("categoryName");
PageInfo pi = (PageInfo) request.getAttribute("pi");
ArrayList<GeneralBulletinComment> list = (ArrayList<GeneralBulletinComment>) request.getAttribute("list");
...
<body id="page-top">
	...
	<div id="wrapper">
		...
		<div id="content-wrapper" class="d-flex flex-column">
			...
			<div id="content">
				...
				<div class="outer">
					...
					<h2 align="center">일반 게시글 댓글 리스트</h2>
					<%}else{ %>
					...
					<br>
					<table align="center">
						<form action="<%=request.getContextPath()%>/search.rguc"
							method="get"
					...
					<table align="center" class="list-area" border="1"
						id="comment-board-comment-repotr-list-td">
						<thead>
							<tr>
						...
						<tbody>
							...
							%>
							<%
							for (GeneralBulletinComment gbc : list) {
							%>
							<tr class="boardReport">
								<td><%=gbc.getReplyNo()%></td>
								<td><%=gbc.getBoardCategoty()%></td>
								<td><%=gbc.getTitle()%></td>
								<td><%=gbc.getReplyComment()%></td>
								<td><%=gbc.getReplyWriter()%></td>
								<td>
									<%
									if (gbc.getGroupMemNoStatus() == 1) {
									%> 이용중인 회원 <%
									} else {
									%> 탈퇴한 회원 <%
									}
					...
					<div align="center" class="paging-area">
						...
						%>
						<button
							onclick="location.href='<%=request.getContextPath()%>/list.rguc?currentPage=1'">&lt;&lt;</button>
						<%
						} else {
						%>
						<button
							onclick="location.href='<%=request.getContextPath()%>/search.rguc?currentPage=1&category=<%=category%>&comment_Status=<%=comment_Status%>'">&lt;&lt;</button>
						<%
						}
						%>
						...
						%>
						<button
							onclick="location.href='<%=request.getContextPath()%>/list.rguc?currentPage=<%=currentPage - 1%>'">&lt;</button>
						<%
						} else {
						%>
						<button>&lt;</button>
						...
						%>
						<button
							onclick="location.href='<%=request.getContextPath()%>/search.rguc?currentPage=<%=currentPage - 1%>&category=<%=category%>&comment_Status=<%=comment_Status%>'">&lt;</button>
						<%
						} else {
						%>
						<button>&lt;</button>
						...
						<!-- 페이지 이동 가능하게끔 -->
						<button
							onclick="location.href='<%=request.getContextPath()%>/list.rguc?currentPage=<%=p%>'"><%=p%></button>
						<%
						} else {
						%>
						<!-- 페이지 이동 불가능하게끔 -->
						...
						<!-- 페이지 이동 가능하게끔 -->
						<button
							onclick="location.href='<%=request.getContextPath()%>/list.rguc?currentPage=<%=p%>&category=<%=category%>&comment_Status=<%=comment_Status%>'"><%=p%></button>
						<%
						} else {
						%>
						<!-- 페이지 이동 불가능하게끔 -->
						...
						%>
						<button
							onclick="location.href='<%=request.getContextPath()%>/list.rguc?currentPage=<%=maxPage%>'">&gt;&gt;</button>
						<%
						} else {
						%>
						<button
							onclick="location.href='<%=request.getContextPath()%>/search.rguc?currentPage=<%=maxPage%>&category=<%=category%>&comment_Status=<%=comment_Status%>'">&gt;&gt;</button>
						<%
						}
						%>
					...
					<div class="modal">
						<div class="modal_body">
							<h2 align="center">일반 게시글 댓글 관리 창</h2>

							<hr>
							<form action="<%=request.getContextPath()%>/report.rguc"
								method="get">
								<table border="1" align="center" class="rguc" id="rguc">
									<input type="hidden" name="userId">
									<input type="hidden" name="CommentNo">
									<input type="hidden" name="common_comment_title">
									<input type="hidden" name="common_comment">
									<tbody>
										<tr>
											<th>카테고리</th>
											<td id="rguc_common_category"></td>
											<th width="120">댓글 번호</th>
											<td width="60" id="rguc_common_board_comment_no"></td>
										</tr>
										<tr>
											<th>댓글이 달린 게시글 제목</th>
											<td colspan="3" id="rguc_common_board_comment_title"></td>
										</tr>
										<tr>
											<th>댓글 작성자</th>
											<td colspan="3" id="rguc_common_board_comment_user"></td>
										</tr>
										<tr>
											<th>댓글 내용</th>
											<td colspan="3" id="rguc_common_board_comment_content">
												<p></p>
											</td>
										</tr>
										<tr>
		...
		<script>
                    ...
                    const btnOpenModal=document.querySelector('.list-area');
		...
		$(function() {
			...
			$("#comment-board-comment-repotr-list-td>tbody>tr").click(function() {
				// console.log("잘 클릭되나??")
				...
				$("#rguc>input[name=CommentNo]").val(bno);
				
				$("#rguc>input[name=userId]").val(CommentUser); 
				$("#rguc>input[name=common_comment_title]").val(boardTitle);
				$("#rguc>input[name=common_comment]").val(commentContent);
				
				
				$("#rguc_common_category").text(category);
				$("#rguc_common_board_comment_no").text(CommentNo);
				$("#rguc_common_board_comment_title").text(boardTitle);
				$("#rguc_common_board_comment_user").text(CommentUser);
				$("#rguc_common_board_comment_content").text(commentContent);
