...
/**
 * Servlet implementation class SearchReularUser
 */
@WebServlet("/search.rul")
public class SearchReularUser extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SearchReularUser() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount =  new AdminService().searchSelectListCount(searchUser);
		...
		ArrayList<RegularUser> list =  new AdminService().searchSelectList(pi, searchUser);
		
		if(searchUser == " " ) {
			request.setAttribute("pi", pi);
			request.setAttribute("list", list);
			
			RequestDispatcher view = request.getRequestDispatcher("views/admin/regularUserList.jsp");
