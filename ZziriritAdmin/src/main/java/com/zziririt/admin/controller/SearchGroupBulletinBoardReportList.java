...
/**
 * Servlet implementation class SearchGroupBulletinBoardReportList
 */
@WebServlet("/search.gbbrl")
public class SearchGroupBulletinBoardReportList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SearchGroupBulletinBoardReportList() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount =  new AdminService().searchSelectGruopBoardListCount(category, boardStatus);
		...
		ArrayList<GroupBoard> list =  new AdminService().searchSelectGruopBoardList(pi, category, boardStatus);
		
		if(category == " " && boardStatus == " ") {
			request.setAttribute("categoryName", categoryName);
			request.setAttribute("pi", pi);
			request.setAttribute("list", list);
			
			request.getRequestDispatcher("views/admin/groupBulletinBoardReportList.jsp").forward(request, response);	
		}else if(boardStatus == " ") {
		...
		request.getRequestDispatcher("views/admin/groupBulletinBoardReportList.jsp").forward(request, response);
			...
			request.setAttribute("boardStatus", boardStatus);
			request.setAttribute("categoryName", categoryName);
			request.setAttribute("pi", pi);
			request.setAttribute("list", list);
